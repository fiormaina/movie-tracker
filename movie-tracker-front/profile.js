(() => {
  ...
  const API_BASE_URL = String(window.MovieTrackerConfig?.apiBaseUrl ?? "")
  ...
  function buildStats(user, publicFolders) {
    const itemsCount = publicFolders.reduce((count, folder) => count + folder.itemsCount, 0);

    return [
      { id: "public-folders", value: publicFolders.length, label: "Публичных папок", icon: "folders" },
      { id: "public-items", value: itemsCount, label: "Элементов в подборках", icon: "items" },
      { id: "following", value: user.followingCount, label: "Подписок", icon: "following" },
      { id: "followers", value: user.followersCount, label: "Подписчиков", icon: "followers" },
    ];
  }
  ...
  function syncProfileLocation(user, isOwner) {
    ...
    const nextUrl = new URL(
      isOwner ? "./profile.html" : getProfileUrl(user.username, false),
  ...
  async function loadProfileView() {
    ...
    setState((currentState) => ({
      ...
      user: profileUser,
      publicFolders,
      stats: buildStats(profileUser, publicFolders),
      pendingFollow: false,
  ...
  function getStatIcon(type) {
    const icons = {
      folders: `<path d="M2.8 5.1H6.6L8 6.7H15.2V13.2C15.2 13.97 14.57 14.6 13.8 14.6H4.2C3.43 14.6 2.8 13.97 2.8 13.2V5.1Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"></path>`,
      items: `<path d="M4.2 4.2H13.8V13.8H4.2V4.2Z" stroke="currentColor" stroke-width="1.5"></path><path d="M7 4.2V13.8M11 4.2V13.8M4.2 8.8H13.8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>`,
      following: `<path d="M6.2 10.3C4.6 9.7 3.4 8.3 3.4 6.6C3.4 4.38 5.18 2.6 7.4 2.6C9.62 2.6 11.4 4.38 11.4 6.6C11.4 8.3 10.2 9.7 8.6 10.3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path><path d="M2.8 14.8C3.45 12.84 5.24 11.5 7.4 11.5C8.71 11.5 9.88 11.99 10.78 12.81" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path><path d="M12.2 9.6H15.2M13.7 8.1V11.1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>`,
      followers: `<circle cx="6.1" cy="6.1" r="2.9" stroke="currentColor" stroke-width="1.5"></circle><circle cx="12.1" cy="7.3" r="2.2" stroke="currentColor" stroke-width="1.5"></circle><path d="M2.8 14.6C3.36 12.64 5.18 11.3 7.4 11.3C9.62 11.3 11.44 12.64 12 14.6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path><path d="M11.2 13.8C11.67 12.74 12.68 12 13.86 12C14.47 12 15.04 12.2 15.5 12.54" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>`,
    };

    return icons[type] ?? icons.folders;
  }
        ...
        <div class="profile-section__head">
          <div>
            ...
            <p class="profile-section__hint">${escapeHtml(
              state.isOwner
                ? "Сводка по вашему профилю, подпискам и публичным папкам."
                : "Сводка по профилю пользователя и его публичным папкам.",
            )}</p>
          </div>
        </div>
    ...
    return `
      ...
      >
        ...
        <div class="folder-card__body">
          ...
          <p class="profile-public-card__count">${escapeHtml(String(folder.count))} ${getItemWord(folder.count)}</p>
  ...
  function renderStateCard(title, text, actionLabel = "", actionUrl = "./profile.html") {
    const actionButton = actionLabel
      ? `<button class="profile-button profile-button--primary" type="button" data-nav-url="${escapeHtml(actionUrl)}">${escapeHtml(actionLabel)}</button>`
      : "";

    return `
      <section class="profile-empty-state profile-empty-state--page" aria-live="polite">
  ...
  async function toggleFollow() {
    ...
    try {
      ...
      setState((currentState) => ({
        ...currentState,
        user: updatedUser,
        stats: buildStats(updatedUser, currentState.publicFolders),
        pendingFollow: false,
      }));
      showToast(updatedUser.isFollowing ? "Подписка оформлена" : "Подписка отменена", "success");
    } catch (error) {
      console.error(error);
      setState((currentState) => ({
        ...currentState,
